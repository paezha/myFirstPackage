...
The name of the file is `Energy and Emissions.xlsx`, stored in folder `data-raw`, and it contains two sheets, as follows:
...
Filter rows with NAs in Continent:
```{r}
...
Check that all NAs have been fixed:
```{r}
summary(energy_and_emissions)
```

There are still two NAs in continent; check:
```{r}
energy_and_emissions |>
  filter(is.na(Continent))
```
...
My table is now complete. I can now save to data folder:
```{r}
